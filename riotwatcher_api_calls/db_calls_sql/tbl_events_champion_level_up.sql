...
--DELETE [DataScience].[dbo].[tbl_events_champion_level_up]

SELECT COUNT(DISTINCT match_id) FROM [DataScience].[dbo].tbl_events_champion_level_up
...
DROP TABLE IF EXISTS #events_champion_level_up
...
INTO #events_champion_level_up
FROM [DataScience].[dbo].tbl_events_champion_level_up ward_kill
LEFT JOIN (
	SELECT DISTINCT
		match_id
		,match_start_date
